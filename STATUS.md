...
|---|---|---|---|
| Engine C++ (MSVC local, GCC CI) | ✅ | gtests **61/61** (2 hashes ancres) ; CI verte jusqu'a 1a0fce4, verdict ring v12 en vol | WASAPI partage/**exclusif** (`--exclusive --buffer-size`, 16 ms a 256), blocs internes 256, **CONTRAT DE PERIODE CLOS** : `--buffer-size` arrondi au multiple de 256 en clair, periode negociee non multiple de 256 = REFUS de demarrer (message actionnable ; exclusif 128 et plancher partage 374 refuses), depth > 6 = refus ; hote VST3 hors processus (**ring v12** : 8 slots = depth <= 6, estampilles par slot — slot perime ou entree dechiree = DRY + compte ; FIFO MIDI GENERIQUE note/CC/pitch-bend au format fil, CC/PB -> parametres via IMidiMapping cote enfant ; **ProcessContext** rempli : position du bloc, tempo entier du document, play/stop, 4/4 ; un controleur d'edition par instance, partage avec la fenetre ; enfant en MMCSS Pro Audio ; `--params` liste les parametres ; cold-restart, GUI a la demande TOPMOST, scan `--vst3-dir` cache) ; **ENTREE MIDI LIVE (Vague 3, maillon 1)** : `--midi-in <port>` (WinMM ; `--list-midi-devices`, `--midi-track <id>` sinon auto = premiere piste avec instrument) -> file SPSC -> callback (drain par sous-bloc, latence de FILE mesuree, `midi-in stats:` toutes les 5 s) -> instrument de la piste cible (offset 0, mute = all-notes-off), graphe traite a l'ARRET quand arme et la timeline se tait (etape 0) ; outil `midi_send` ; spec `midi-in.spec.ts` = matrice **DX10 / Dexed / Surge XT VERTE** sur le vrai port loopMIDI « MagicPotion » (CC64 + pitch-bend via IMidiMapping, latence de file 0,3-17 ms, pipeline 10,7 ms en exclusif 256). **MiniLab 3 joue a l'oreille** (port « Minilab3 MIDI », file 2,8 ms, 0 underrun, gresillement corrige : enfant plugin_host en **MMCSS Pro Audio** — 1356 blocs rates -> 0 sur 20 notes ; partage 512 = stack quotidienne, l'exclusif prend le device et coupe Twitch). Le « transitoire DX10 » etait un gel de 2 s de la boucle de controle par PUT d'asset (`localhost` -> `::1`, corrige `util/net_loopback.h`) ; `control-loop stall:` loggue tout gel > 50 ms. Dette : aucun limiteur de sortie (point produit), 5 natifs (utility/eq3/comp/drive/delay), stems + etat de plugin au store, PDC ecrivain (lecteur de stem avance de la latence declaree), rendu offline deterministe + `--probe` par etage, export sur thread ouvrier, MIDI note-on/off sample-accurate, Session (launch quantise, horloge libre), automation gain/pan/master (miroir exact du TS), noyau tempo entier `tempo.h` == `tempo.ts`, `resolveMusicalTime` = point d'etranglement, bascule de projet (SwitchProject), crash handler auto-symbolisant, `timeBeginPeriod(1)` |
