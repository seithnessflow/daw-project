...
| 4 | LNA HTTPS→WS local | ⚠️ TIENT SOUS CONDITIONS (2026-08-23, test des mains) | Prouve : page HTTPS publique (tunnel trycloudflare) -> invite Chrome APPARUE, autorisee -> WS CONNECTE au moteur local. Le tunnel n'a servi QUE la page : l'URL WS etait 127.0.0.1, le socket n'a jamais touche le tunnel — le mecanisme est la carve-out loopback « potentially trustworthy » de Chrome (ws:// depuis https:// tolere vers loopback), par construction. CONDITIONS OUVERTES : (1) --allow-origin etait un drapeau de test — en prod l'origine du site doit etre connue du moteur (design a trancher : compilee ou configuree a l'installation) ; (2) handshake prouve, PAS l'audio/telemetrie a travers ce chemin (vraie appli depuis l'origine publique = preuve suivante) ; (3) Firefox/Safari non testes ; (4) refus/memoire/annulation a documenter (contre-epreuve en cours). Canari fetch NON CONCLUSIF chez nous (400 sans CORS -> « Failed to fetch » meme quand le reseau passe — garder ecrit : resservira au premier faux diagnostic reseau). Le moteur ne loggue pas les connexions acceptees (a corriger) |
